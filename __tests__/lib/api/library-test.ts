...
describe('getLibraryAlbums', () => {
  it('calls GET /library/albums with artistId param', async () => {
    ...
    expect(mockApi.get).toHaveBeenCalledWith('/library/albums', { params: { artistId: '42' } });
...
describe('getLibraryTracks', () => {
  it('calls GET /library/tracks with albumId param', async () => {
    ...
    expect(mockApi.get).toHaveBeenCalledWith('/library/tracks', { params: { albumId: '99' } });
...
describe('getAlbumCover', () => {
  it('calls GET /artists/release-group/:mbid/cover', async () => {
    ...
    expect(mockApi.get).toHaveBeenCalledWith('/artists/release-group/def-456/cover');
    expect(result).toEqual(cover);
  });
});
...
describe('triggerAlbumSearch', () => {
  it('calls POST /library/downloads/album/search', async () => {
    ...
    expect(mockApi.post).toHaveBeenCalledWith('/library/downloads/album/search', { albumId: '99' });
...
describe('deleteAlbum', () => {
  it('calls DELETE /library/albums/:id with deleteFiles param', async () => {
    ...
    expect(mockApi.delete).toHaveBeenCalledWith('/library/albums/99', { params: { deleteFiles: false } });
    expect(result).toEqual({ success: true });
  });
  ...
  it('passes deleteFiles=true when specified', async () => {
    ...
    expect(mockApi.delete).toHaveBeenCalledWith('/library/albums/99', { params: { deleteFiles: true } });
  });
});
