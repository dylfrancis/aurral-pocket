...
describe('SecureStorage.user', () => {
  ...
  it('sets user JSON', async () => {
    ...
    expect(mockSecureStore.setItemAsync).toHaveBeenCalledWith('user_json', '{"id":1}');
  });
...
describe('AppStorage', () => {
  ...
  it('sets server URL', async () => {
    ...
    expect(mockAsyncStorage.setItem).toHaveBeenCalledWith('server_url', 'https://example.com');
  });
