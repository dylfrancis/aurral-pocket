...
describe('SearchArtistRow', () => {
  it('renders artist name', () => {
    const { getByText } = render(
      <SearchArtistRow artist={baseArtist} isInLibrary={false} onPress={() => {}} />,
    );
    expect(getByText('Radiohead')).toBeTruthy();
  });

  it('renders sort-name when different from name', () => {
    const artist = { ...baseArtist, name: 'The Beatles', 'sort-name': 'Beatles, The' };
    const { getByText } = render(
      <SearchArtistRow artist={artist} isInLibrary={false} onPress={() => {}} />,
    );
  ...
  it('shows "In Library" chip when isInLibrary is true', () => {
    const { getByText } = render(
      <SearchArtistRow artist={baseArtist} isInLibrary={true} onPress={() => {}} />,
    );
  ...
  it('calls onPress when pressed', () => {
    ...
    const { getByText } = render(
      <SearchArtistRow artist={baseArtist} isInLibrary={false} onPress={onPress} />,
    );
