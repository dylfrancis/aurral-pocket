...
describe('AlbumStatusBadge', () => {
  it('shows "Complete" when percentOfTracks >= 100', () => {
    const album = { ...baseAlbum, statistics: { trackCount: 10, sizeOnDisk: 500, percentOfTracks: 100 } };
    const { getByText } = render(<AlbumStatusBadge album={album} />);
    expect(getByText('Complete')).toBeTruthy();
  });

  it('shows "Complete" when sizeOnDisk > 0', () => {
    const album = { ...baseAlbum, statistics: { trackCount: 10, sizeOnDisk: 100, percentOfTracks: 50 } };
    const { getByText } = render(<AlbumStatusBadge album={album} />);
    expect(getByText('Complete')).toBeTruthy();
  });
