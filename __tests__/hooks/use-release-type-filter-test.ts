...
describe('matchesFilter', () => {
  ...
  it('matches when all types are selected', () => {
    const album = { ...baseAlbum, albumType: 'Album' as const, secondaryTypes: [] };
  ...
  it('matches EP type', () => {
    ...
    const album = { ...baseAlbum, albumType: 'EP' as const, secondaryTypes: [] };
    expect(matchesFilter(album, selected)).toBe(true);
  });

  it('matches Single type', () => {
    const selected = new Set(['Single', ...SECONDARY_TYPES]);
    const album = { ...baseAlbum, albumType: 'Single' as const, secondaryTypes: [] };
    expect(matchesFilter(album, selected)).toBe(true);
  });
